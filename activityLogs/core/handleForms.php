    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) && strlen($password) >= 6) {
        ...
        if ($stmt->fetchColumn() > 0) {
            ...
            if (registerUser($firstName, $lastName, $username, $email, $hashedPassword)) {
                logActivity(null, 'INSERT', "Registered new user: $username", null, $username);
...
function handleUserLogin() {
    ...
    if (!empty($username) && !empty($password)) {
        if ($userID = loginUser($username, $password)) {
            ...
            logActivity($userID, 'LOGIN', "User logged in: $username", null, $username);
...
function handleCreateApplicant() {
    ...
    if (createApplicant($userID, $firstName, $lastName, $cause, $skills, $experience, $addedBy)) {
        logActivity($userID, 'INSERT', "Created new applicant: $firstName $lastName", null, $_SESSION['username']);
...
function handleDeleteApplicant() {
    ...
    if (deleteApplicant($applicationID)) {
        logActivity($userID, 'DELETE', "Deleted applicant ID: $applicationID", null, $_SESSION['username']);
...
function handleUpdateApplicant() {
    ...
    if (updateApplicant($applicationID, $firstName, $lastName, $cause, $skills, $experience, $userID)) {
        logActivity($userID, 'UPDATE', "Updated applicant ID: $applicationID", null, $_SESSION['username']);
